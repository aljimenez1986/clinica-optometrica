...
export default function TestsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const listaRes = await api.testResultados.list(undefined, { all: true })
        const resList = Array.isArray(listaRes) ? listaRes : []
        if (resList.length === 0) {
          setResultados([])
          setLoading(false)
          return
  ...
  async function recalcularValoresOptopad() {
    ...
    try {
      ...
      for (const r of optopadResults) {
        ...
        await api.testResultados.update(r.id, { datos_respuesta: nuevosDatos })
      }
      const lista = Array.isArray(await api.testResultados.list(undefined, { all: true })) ? await api.testResultados.list(undefined, { all: true }) : []
      if (lista.length > 0) {
        const pIds = [...new Set(lista.map((x: any) => x.paciente_id).filter(Boolean))]
